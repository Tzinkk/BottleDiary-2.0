...
/**
 * Analyzes the wine label image directly from the client side using the gemini-2.5-flash model.
...
export async function analyzeWineLabel(imageUri: string): Promise<Partial<WineBottle> & { mainTastingNotes?: string }> {
  console.log("[AI Service] Client-side starting label scan using gemini-2.5-flash...");
...
/**
 * Generates a random multiple choice question client-side using gemini-2.5-flash.
 */
export async function generateQuizQuestion(): Promise<QuizQuestion> {
  console.log("[AI Service] Client-side generating quiz question using gemini-2.5-flash...");
  const ai = getAI();
