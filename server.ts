...
app.post("/api/scan-wine", async (req, res) => {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      return res.status(500).json({ 
        error: "Gemini API key is not configured. Please add GEMINI_API_KEY in the app settings or environment variables." 
      });
    }
...
// Mount Vite middleware only in development & when not deployed as Vercel serverless function
if (process.env.NODE_ENV !== "production" && !process.env.VERCEL) {
  ...
  // Production or Vercel environment: Serve static files from dist/ folder
...
// Start local dev server if not executing as a Vercel serverless function
